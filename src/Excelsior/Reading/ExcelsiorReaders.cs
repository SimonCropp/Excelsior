...
/// <summary>
/// Typed cell readers consumed exclusively by the code that
/// <c>Excelsior.SourceGenerator.SheetBuilderGenerator</c> emits — there are
/// no hand-written call sites in the runtime. Method names and signatures
/// here are an API contract with the generator (see
/// <c>SheetBuilderGenerator.cs</c> ~lines 1000-1018, where calls like
/// <c>global::Excelsior.ExcelsiorReaders.ReadString(...)</c>,
/// <c>ReadInt</c>, <c>ReadEnum&lt;T&gt;</c>, <c>ReadObject</c> etc. are
/// generated by name); renaming or changing parameter order silently breaks
/// every consumer that uses the source generator.
///
/// Each method reports parse failures via the <see cref="Action{T1, T2}"/>
/// callback (slot index + message) and returns a default value, mirroring
/// the reflection-based <see cref="CellConverter"/> path's "row keeps
/// loading; errors collected" semantics.
/// </summary>
